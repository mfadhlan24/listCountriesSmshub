...
// Data yang diberikan dalam bentuk string
const dataString = `0	Russia	russia	aiva,any,beeline,center2m,danycom,ezmobile,gazprombank_mobile,lycamobile,matrix,mcn,megafon,motiv,mts,mtt,mtt_virtual,rostelecom,sber,simsim,tele2,tinkoff,ttk,vtb_mobile,winmobile,yota
...
rows.forEach((rowString, index) => {
  // Jika ini adalah baris pertama, itu adalah kepala tabel
  // const isHeader = index === 0;

  // Buat elemen baris baru
  const rowElement = document.createElement('tr');

  // Split string baris menjadi kolom
  const columns = rowString.split('\t');

  columns.forEach((columnString, columnIndex) => {
    // Buat elemen kolom baru
    const columnElement = document.createElement('td')

    // Masukkan teks ke dalam kolom
    columnElement.textContent = columnString;

    // Masukkan kolom ke dalam baris
...
// Tambahkan kelas khusus untuk kolom operator agar dapat di-scroll
const operatorColumn = document.querySelector('.table-scrollable');
operatorColumn.classList.add('table-scrollable');

// Tambahkan event listener untuk menyalin nilai saat sel diklik
document.querySelectorAll('td').forEach(td => {
  td.addEventListener('click', () => {
    const text = td.textContent.trim();
    navigator.clipboard.writeText(text)
      .then(() => {
        alert('Nilai telah disalin: ' + text);
      })
      .catch(err => {
